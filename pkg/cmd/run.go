...
doppler run --mount secrets.json -- cat secrets.json`,
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		watchHandler := func(data []byte) {
			...
			if err != nil {
				controllers.CaptureEvent("WatchDataParseError", nil)
			...
			if watchSecrets.Type != "ping" {
				controllers.CaptureEvent("WatchDataReceived", map[string]interface{}{"event": watchSecrets.Type})
			}

			if watchSecrets.Type == "secrets.update" {
				...
				utils.LogDebug(fmt.Sprintf("Received %s event", watchSecrets.Type))
				...
				startProcess()
			} else if watchSecrets.Type == "connected" {
				utils.LogDebug("Connected to secrets stream")
			} else if watchSecrets.Type == "ping" {
				// do nothing
			} else {
				utils.Log(fmt.Sprintf("Received unknown event: %s", watchSecrets.Type))
			}
		}
